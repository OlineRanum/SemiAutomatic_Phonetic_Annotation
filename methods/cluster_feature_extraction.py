class AutoAnnotate:
    ...
    def build_database(self, permformer = 'Daniel'):
                ...
                with open(metapath, 'r') as file:
                    ...
                    if name == permformer:
                        ...
                        if df is not None:
                            ...
                            spectrum = self.build_features(df)
        ...
        merged_df = pd.merge(spectra_df, annotator.semlex, on='label', how='inner')
        
        df = self.k_means(glosses, merged_df)
                # Select the columns you want to write to the CSV file
        columns_to_write = df[['label', 'new_labels', 'Handshape']]

        # Write the selected columns to a CSV file
        columns_to_write.to_csv('output.csv', index=False)
    ...
    def k_means(self, glosses, df):
            k = len(df['Handshape'].unique())
            kmeans = KMeans(n_clusters=k)
            expanded_df = df['spectrum'].apply(pd.Series)
            data = expanded_df.to_numpy()
            ...
            """
            plt.hist(labels, bins='auto')  # 'auto' lets matplotlib decide the best bin size
            plt.title('Histogram of new_labels L')
            plt.xlabel('Value')
            ...
            handshapes = df['Handshape'].values
            plt.hist(handshapes, bins='auto')  # 'auto' lets matplotlib decide the best bin size
            plt.title('Histogram of handshapes L')
            plt.xlabel('Value')
            plt.ylabel('Frequency')
            plt.show()
            """
