class AutoAnnotate:
    ...
    def minmax_normalize(self, data):
        ...
        Parameters:
        - data: List of numbers to be normalized.
        
        Returns:
        - A list of normalized numbers in the range [0, 1].
        """
        min_val = min(data)
        max_val = max(data)
        if max_val - min_val == 0:  # Avoid division by zero
            return [0 for _ in data]  # Returning all zeros if all values are the same
    ...
    def build_spectra(self, df):
        spectrum = np.zeros(len(df))
        for col_name, data in df.items():
            spectrum += data.values
            
        # Normalize by number of frames
        spectrum /= len(df.keys())
            ...
            try:
                ...
                with open(metapath, 'r') as file:
                    ...
                    if name == permformer:
                        ...
                        if df is not None:
                            spectrum = self.build_spectra(df)
